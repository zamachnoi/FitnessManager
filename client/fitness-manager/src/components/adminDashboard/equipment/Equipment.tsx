...
async function toggleMaintenance(equipmentId: number, maintenance: boolean) {
  if(maintenance) {
    const res = await patchData(`admin/equipment/${equipmentId}/endMaintenance`, {})
    return res
  }
  else {
    const res = await patchData(`admin/equipment/${equipmentId}/startMaintenance`, {})
    return res
  }
...
const Equipment = ({
  ...
  return (
		...
		<div className="flex flex-row w-[650px]">
          ...
          <p>{moment(equipment.last_maintained).format("MM/DD/YYYY")}</p>
          {maintenance ? (
            <Button className="w-[150px]" onClick={() => handleMaintenance()}>
              Remove Maintenance
            </Button>
          ) : (
            <Button className="w-[150px]" onClick={() => handleMaintenance()}>
