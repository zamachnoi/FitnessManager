...
const TrainerScheduleCard = () => {
	...
	async function getTrainerData() {
		const res = await getData(`trainers/${userId}`)
		return res.data
	}

	async function updateTrainerData(data: TrainerUpdateRequest) {
		const res = await patchData(`trainers/${userId}`, data)
		return res.data
	}
	...
	useEffect(() => {
		getTrainerData().then((data: TrainerType) => {
			const newValues = {
				start_availability: parseInt(
					moment(data.start_availability, "HH:mm:ss").format("HH")
				),
				end_availability: parseInt(
					moment(data.end_availability, "HH:mm:ss").format("HH")
				),
				rate: data.rate,
			}
